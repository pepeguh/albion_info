        ...
        <div className="render_div">
         {render.map((item, index)=>(
           ...
           <div className="prices_div"> 
            <div className={`Bridgewatch ${item.sell_price_minBridgewatch === 0 ? '' : 'm_p'}`}> 
            {item.sell_price_minBridgewatch==0?null:
            
            item.sell_price_minBridgewatch
            }
            </div>
            <div className={`Caerleon ${item.sell_price_minCaerleon === 0 ? '' : 'm_p'}`}>
              {item.sell_price_minCaerleon==0?null:
              
            item.sell_price_minCaerleon
              }
            </div>
            <div className={`Fort_Sterling ${item[`sell_price_minFort Sterling`] === 0 ? '' : 'm_p'}`}>
              {item[`sell_price_minFort Sterling`]==0?null:
              
            item[`sell_price_minFort Sterling`]
              }
            </div>
            <div className={`Lymhurst ${item.sell_price_minLymhurst === 0 ? '' : 'm_p'}`}>
              {item.sell_price_minBLymhurst==0? null:
              
            item.sell_price_minLymhurst
              }
            </div>
            <div className={`Martlock ${item.sell_price_minMartlock=== 0 ? '' : 'm_p'}`}>
              {item.sell_price_minMartlock==0 ? null:
              
            item.sell_price_minMartlock
              }
            </div>
            <div className={`Thetford ${item.sell_price_minThetford === 0 ? '' : 'm_p'}`}>
              {item.sell_price_minThetford==0?null:
